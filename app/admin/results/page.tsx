  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-6 md:px-8 md:py-10">
      <div className="max-w-md md:max-w-5xl mx-auto">
        <AppHeader title="Confirm winners" backHref="/admin" showLogo />
            ...
            <div className="md:grid md:grid-cols-2 md:gap-4">
              {[1, 2].map((i) => (
                <div key={i} className="bg-white border border-slate-200 rounded-xl p-4 mb-4 md:mb-0 animate-pulse">
                  ...
                  <div className="h-9 bg-slate-100 rounded-lg w-full mb-2" />
                  <div className="h-9 bg-slate-100 rounded-lg w-full" />
                </div>
              ))}
            </div>
            <div className="h-11 md:max-w-sm bg-slate-100 rounded-lg mt-4 animate-pulse" />
          </>
        ) : (
        <>
        ...
        {error && <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2 mb-4 mt-4 md:max-w-sm">{error}</p>}
        {message && <p className="text-sm text-green-700 bg-green-50 border border-green-200 rounded-lg px-3 py-2 mb-4 mt-4 md:max-w-sm">{message}</p>}

        <button
          onClick={finalize}
          disabled={!allConfirmed}
          className="w-full md:max-w-sm mt-4 bg-blue-900 text-white font-medium py-3 rounded-lg active:bg-blue-800 disabled:opacity-40"
        >
